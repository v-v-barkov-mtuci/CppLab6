...
namespace Containers {
    const int t = 2;
    export template<class T> class Tree {
    public:
        struct BNode {
            T keys[2 * t];
    ...
    private:
        BNode* root;
        void insert_to_node(T key, BNode* node) {
        ...
        bool searchKey(T key, BNode* node) {
            if (node != nullptr) {
                if (node->leaf == false) {
                    int i;
                    for (i = 0; i <= (2 * t - 1); i++) {
        ...
        void removeFromNode(T key, BNode* node) {
        ...
        void removeLeaf(T key, BNode* node) {
